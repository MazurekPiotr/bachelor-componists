<p>
    You have been mentioned by <a href="{{ env('APP_URL') . '/user/profile/@' . $post->user->name }}">{{ '@' . $post->user->name }}</a> in a post from the <a href="{{ env('APP_URL') . 'projects' . $topic->slug . '#post-' . $post->id }}">{{ $topic->title }}</a> topic @ {{ env('APP_NAME') }}.
</p>
<p>
